...
from srp_analyzer import (
    SRPAnalyzer,
    SRPViolation,
    analyze_file,
    analyze_directory
)
from __init__ import SRPThresholds, DEFAULT_SRP_THRESHOLDS, Severity
...
class UserManager:
    ...
    def authorize_user(self): pass  # 8 methods - over threshold of 7
        ...
        assert any("Too many methods" in reason for reason in analyzer.violations[0].reasons)
...
class UserManagerAndEmailer:
    ...
    def format_email(self): pass
        ...
        assert any("Multiple responsibility groups" in reason for reason in violation.reasons)
...
class LargeClass:
    {methods}
        ...
        assert len(analyzer.violations) > 0
        assert any("Class too large" in reason for reason in analyzer.violations[0].reasons)
...
class UserAndOrderManager:
    def manage(self): pass
        ...
        assert len(analyzer.violations) == 1
        assert any("Class name contains 'and'" in reason for reason in analyzer.violations[0].reasons)
...
class MinorViolation:
    ...
    def method8(self): pass  # Just over threshold
        ...
        if analyzer.violations:
            assert analyzer.violations[0].severity == Severity.INFO or \
                   analyzer.violations[0].severity == Severity.WARNING
...
class TestClass:
    ...
    def test_method2(self): pass
            ...
            assert len(violations) >= 1
            assert any(v.class_name == "ComplexClassWithManyResponsibilities" 
                      for v in violations)
