...
def retry():
    return MAX_RETRIES
        ...
        violations = [v for v in detector.violations 
                     if v.value in [5, 30, "secret-key-123"]]
        assert len(violations) == 0
...
def get_config():
    ...
    return config["database"]
        ...
        string_violations = [v for v in detector.violations 
                            if v.value in ["database", "port", "timeout"]]
        assert len(string_violations) == 0
        
        # But numeric values should be detected
        numeric_violations = [v for v in detector.violations 
                            if v.value in [5432, 30]]
...
class TestMagicNumberViolation:
    ...
    def test_suggestion_generation_for_strings(self):
        ...
        violation = MagicNumberViolation("test.py", 10, 5, "very long string that should be truncated", "in main")
